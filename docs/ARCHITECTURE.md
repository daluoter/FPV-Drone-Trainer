...
Phase 8 implements the controller boundary, canonical Actual Rates, a modular SI angular-rate PID loop, deterministic controller-to-simulation runtime, the Free Flight presentation seam, bounded tuning, four geometric training evaluators, and bounded replay. The application-owned poller is sampled by the runtime's requestAnimationFrame owner; React receives throttled telemetry only. The direct Three.js scene contains ground/grid, a takeoff pad, hover target, turn gates, Split-S reference, orbit tower/circle, bounded training path, a live quad, and an independent replay ghost, with rigid FPV, chase and free/debug cameras. The explicit RC arm handoff consumes the Controller Lab's verified profile/session and checks current neutral/low throttle only at arm time. Developer keyboard input is opt-in and follows the same normalized RC/rates/PID path. Tuning edits a copied, validated versioned configuration and reaches flight only through an explicit disarmed reset/reconfiguration boundary. Training consumes one per-fixed-step sample hook, carries explicit armed telemetry, uses O(1) incremental evaluator state with a fixed-capacity decimated/ring path snapshot, and never mutates physics. Replay records only compact decimated fields from armed fixed-step samples, retains one bounded in-memory last flight, and interpolates an immutable ghost without writing the live `DroneState`.
...
- **Replay** stores compact immutable decimated samples from actual armed fixed-step callbacks in a bounded ring. Playback owns a relative clock and independent `ReplayGhostState`; shortest-path quaternion interpolation, trajectory drawing, and camera presentation never mutate the player simulation. Entering replay disarms and aborts an active lesson; exit resets timestamps/controller memory and requires explicit rearm. Replay is in-memory only and is not training input.
